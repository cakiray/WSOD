...
def main() -> None:
    ...
    for feed_dict in tqdm(dataflow['test'], desc='eval'):
        ...
        #_subsampled_outputs.append(outputs[:,5:6])
        print("\n\n", feed_dict['file_name'])
        print("\ninput shape:", inputs.F.shape) 
        print("target shape:", targets.shape)
        print("output shape:", outputs[:,5].shape)
        #print(outputs[:20,12])
        
        filename = feed_dict['file_name'][0]
        out = outputs[:,0].cpu()
        pc_sampled = inputs.F.cpu()
        # write voxelized output as numpy
        np.save( os.path.join(configs.out_save_dir, filename.replace('bin', 'npy')), out.detach().numpy())
        # write voxelized point clouds as numpy (used for visualization)
        np.save(os.path.join(configs.out_save_dir, filename.replace('.bin', 'pc.npy')), pc_sampled)
